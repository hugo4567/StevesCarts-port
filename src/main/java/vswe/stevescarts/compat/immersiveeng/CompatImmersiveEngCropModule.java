package vswe.stevescarts.compat.immersiveeng;
...
public class CompatImmersiveEngCropModule implements ICropModule {
    @Override
    public boolean isSeedValid(ItemStack stack) {
        // Détection via tag c:seeds ou namespace immersiveengineering
        TagKey<Item> seedsTag = TagKey.of(Registry.ITEM_KEY, new Identifier("c", "seeds"));
        return stack.isIn(seedsTag) ||
               Registry.ITEM.getId(stack.getItem()).getNamespace().equals("immersiveengineering");
    }

    @Override
    public BlockState getCropFromSeed(ItemStack stack, World world, BlockPos pos) {
        Identifier id = Registry.ITEM.getId(stack.getItem());
        if (id.getNamespace().equals("immersiveengineering")) {
            String cropName = id.getPath().replace("_seeds", "_crop");
            Identifier cropId = new Identifier("immersiveengineering", cropName);
