...
/**
 * Iron Drill Module - Improved drilling capability
 * Better than basic but not as powerful as galgadorian
 */
public class IronDrillModule extends CartModule implements Configurable, Worker {
    private int blocksDestroyed = 0;
    private static final int DRILL_RANGE = 1;  // 1.5 block radius
    private static final int DRILL_HEIGHT = 2;
    private int workTicks = 0;
    private static final int DRILL_INTERVAL = 8;

    public IronDrillModule(CartEntity minecart, ModuleType<?> type) {
        super(minecart, type);
    }
    ...
    public void configure(WPlainPanel panel, CartHandler handler, PlayerEntity player) {
        WLabel title = new WLabel(TextHelper.literal("Iron Drill"));
        panel.add(title, 0, 0);
