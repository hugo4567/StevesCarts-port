...
/**
 * Hardened Drill Module - Heavy-duty drilling
 * Maximum drilling power and range
 */
public class HardenedDrillModule extends CartModule implements Configurable, Worker {
    private int blocksDestroyed = 0;
    private static final int DRILL_RANGE = 3;  // 3 block radius
    private static final int DRILL_HEIGHT = 4;  // 4 blocks high
    private int workTicks = 0;
    private static final int DRILL_INTERVAL = 5;  // Very fast

    public HardenedDrillModule(CartEntity minecart, ModuleType<?> type) {
        super(minecart, type);
    }
        ...
        for (int x = -DRILL_RANGE; x <= DRILL_RANGE; x++) {
            for (int height = 0; height <= DRILL_HEIGHT; height++) {
                for (int range = 1; range <= DRILL_RANGE; range++) {
                    for (int z = -DRILL_RANGE; z <= DRILL_RANGE; z++) {
    ...
    public void configure(WPlainPanel panel, CartHandler handler, PlayerEntity player) {
        WLabel title = new WLabel(TextHelper.literal("Hardened Drill"));
        panel.add(title, 0, 0);
